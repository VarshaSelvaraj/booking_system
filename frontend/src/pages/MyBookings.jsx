        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-20 p-10">
          {bookings.length === 0 ? (
            <div className="col-span-full flex flex-col items-center justify-center p-10">
              ...
              <p className="text-gray-400 text-center">No bookings found. Events you book will appear here.</p>
              <button className="mt-4 px-6 py-2 bg-violet-500 hover:bg-violet-400 text-white rounded-lg transition duration-300 shadow-md" onClick={() => window.location.href = '/dashboard'}>
                Browse Events
              </button>
            </div>
          ) : (
                ...
                >
                  <div className="relative">
                    ...
                    <div className="absolute top-4 right-4 bg-white/80 backdrop-blur-sm px-3 py-1.5 rounded-full shadow-md">
                      <span className="text-sm font-semibold text-violet-800">{formatDate(date)}</span>
                    </div>
                    <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/70 to-transparent h-16"></div>
                  </div>
                  <div className="p-5">
                    <h2 className="text-xl font-bold text-gray-500 mb-3 line-clamp-2">{event_title}</h2>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex justify-center items-center z-50">
          ...
          <div className="bg-white rounded-2xl shadow-lg w-full max-w-2xl p-6 relative animate-fadeIn overflow-hidden mx-4">
            ...
            <div className="space-y-5">
              {/* Event Title */}
              <h2 className="text-2xl font-bold text-violet-900">{selectedBooking.events.event_title}</h2>
              ...
              {selectedBooking.events.description && (
                <div className="bg-white p-5 rounded-xl border border-violet-100 shadow-sm">
                  ...
                  <p className="text-gray-400 whitespace-pre-wrap">{selectedBooking.events.description}</p>
                </div>
              )}
