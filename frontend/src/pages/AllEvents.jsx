...
const AllEvents = () => {
    ...
    const fetchEvents = async () => {
      try {
        const res = await axios.get(
          `${import.meta.env.VITE_API_URL}/events/getevents`,
  ...
  const handleConfirmEnrollment = async (eventId) => {
    try {
      const res = await axios.post(
        `${import.meta.env.VITE_API_URL}/events/enroll`,
        { eventId },
        { withCredentials: true }
      );
      ...
      if (
        err.response?.data?.message === "You are already enrolled in this event"
      ) {
        Swal.fire({
          icon: "info",
  ...
  const calculateEnrollmentStats = (event) => {
    ...
    const enrollmentPercentage = Math.round(
      (event.slots_booked / totalSeats) * 100
    );
    ...
    <div className="relative">
      ...
      <div
        className={`transition-all duration-300 ${
          selectedEvent ? "blur-sm pointer-events-none select-none" : ""
        }`}
      >
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-20 p-10">
          {events.length === 0 ? (
            ...
            events.map((event, index) => {
              const { totalSeats, enrollmentPercentage, isFull } =
                calculateEnrollmentStats(event);
              return (
                <div
                  key={event.id}
              ...
              <div className="mt-2 mb-4">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className="text-sm font-semibold text-violet-800">
                    {selectedEvent.slots_booked} /{" "}
                    {calculateEnrollmentStats(selectedEvent).totalSeats} seats
                    filled
                  </span>
                </div>
                ...
                <div className="w-full bg-gray-200 rounded-full h-3 overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${
                        calculateEnrollmentStats(selectedEvent)
                          .enrollmentPercentage
                      }%`,
                      backgroundColor: calculateEnrollmentStats(selectedEvent)
                        .isFull
                        ? "#ef4444"
                        : calculateEnrollmentStats(selectedEvent)
                            .enrollmentPercentage > 80
                ...
                <div className="space-y-3">
                  ...
                  <p className="text-gray-700 flex items-start">
                    ...
                    <span>
                      ...
                      {selectedEvent.contact_mail || "Not provided"}
                    </span>
                  </p>
                  <p className="text-gray-700 flex items-start">
                    <svg
                    ...
                    <span>
                      ...
                      {selectedEvent.available_slots -
                        selectedEvent.slots_booked}{" "}
                      of {selectedEvent.available_slots} slots
                    </span>
                  </p>
                </div>
              </div>
