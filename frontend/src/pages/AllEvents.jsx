...
const AllEvents = () => {
  ...
  const[slot , setSlots] = useState(0);
  ...
  const imageArray = [
    ...
    "https://assets-global.website-files.com/63062129119620a44791a2eb/639b7845e9be869771e540b8_mural-blog-images.jpg",
    "https://img.freepik.com/premium-photo/people-business-meeting-high-worker_961977-293.jpg"
  ];
  ...
  const handleConfirmEnrollment = async (eventId) => {
    try {
      ...
      if (res.data.success) { 
        // Update the events state to reflect the new slot count
        setEvents(prevEvents => prevEvents.map(event => {
          if (event.id === eventId) {
            return {
              ...event,
              slots_booked: event.slots_booked + 1
            };
          }
          return event;
        }));
  ...
  if (loading) return (
    <div className="flex items-center justify-center h-64">
      <div className="animate-pulse flex flex-col items-center">
        ...
        <p className="text-violet-400">Loading events...</p>
      </div>
    </div>
  );
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-20 p-10">
          {events.length === 0 ? (
            <div className="col-span-full flex flex-col items-center justify-center p-10">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-16 w-16 text-violet-300 mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
              </svg>
              <p className="text-gray-400 text-center">No upcoming events found. Check back later for new events.</p>
            </div>
          ) : (
                ...
                >
                  <div className="relative">
                    ...
                    <div className="absolute top-4 right-4 bg-white/80 backdrop-blur-sm px-3 py-1.5 rounded-full shadow-md">
                      <span className="text-sm font-semibold text-violet-900">{formatDate(event.date)}</span>
                    </div>
                    <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/70 to-transparent h-16"></div>
                  </div>
                  
                  <div className="p-5">
                    <h2 className="text-xl font-bold text-gray-500 mb-3 line-clamp-2">{event.event_title}</h2>
                    <div className="space-y-2">
                      ...
                      <div className="mt-1">
                        <div className="flex justify-between items-center mb-1">
                          ...
                          {isFull ? (
                            <span className="text-xs text-red-800 font-medium">No slots available</span>
                          ) : enrollmentPercentage >= 90 ? (
                            <span className="text-xs text-red-800 font-medium">Almost full!</span>
                          ) : enrollmentPercentage >= 70 ? (
                            <span className="text-xs text-orange-600 font-medium">Filling up quickly</span>
                          ) : (
                            <span className="text-xs text-green-800 font-medium">Spots available</span>
                          )}
                        </div>
                    ...
                    <div className="mt-5">
                      {isFull ? (
                        ...
                        <button 
                          className="w-full text-white py-2.5 px-4 rounded-xl text-sm font-semibold transition duration-300 ease-in-out flex items-center justify-center shadow-md"
            ...
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-violet-50 p-5 rounded-xl">
                <div className="space-y-3">
                  <p className="text-gray-700 flex items-start">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-3 text-violet-400 flex-shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              {selectedEvent.description && (
                <div className="bg-white p-5 rounded-xl border border-violet-100 shadow-sm">
                  <h3 className="text-lg font-semibold text-violet-900 mb-3">Event Description</h3>
                  <p className="text-gray-400 whitespace-pre-wrap">{selectedEvent.description}</p>
                </div>
              )}
